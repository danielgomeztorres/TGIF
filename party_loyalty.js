...
if (diferente == "/senate-party-loyalty.html") {
	diferente = "senate";
}
if (diferente == "/house_party_loyalty.html") {
	diferente = "house";
}
...
var allMembers;
var allMembersRepeat; //duplicamos la array "allmembers " para ordenarlaa y que la primera no sea modificada
...
function tableTop(statistics) {
	...
	function votesParty() {
		...
		td4.innerHTML = statistics.votes_with_party_pct_rep.toFixed(2) + "%";
...
function duplicated1() {
	allMembersRepeat.sort(function (a, b) {
...
function duplicated2() {
	...
	//console.log("duplicatedReverse", allMembersRepeat2);
}
...
function pusha(news) {
	...
	for (var k = resultPct.length; k < news.length; k++) {
		if (resultPct[resultPct.length - 1].votes_with_party_pct == news[k].votes_with_party_pct) { // añadir miembros value igual
...
function bottomTabla(TheMembers) {
	...
	for (var i = 0; i < TheMembers.length; i++) {
		...
		tdmissedvote.innerHTML = TheMembers[i].votes_with_party_pct + "%";
